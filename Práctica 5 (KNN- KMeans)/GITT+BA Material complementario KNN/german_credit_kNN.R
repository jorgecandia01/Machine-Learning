setwd("/Users/jorgecandia/UNIVERSIDAD/TERCERO/Machine Learning/Prácticas R/Práctica 5 (KNN- KMeans)/GITT+BA Material complementario KNN")
...
train.gc <- gc.subset.n[dat.d,] # 70% training data
test.gc <- gc.subset.n[-dat.d,] # remaining 30% test data
...
k.max = match(max(k.optm),k.optm) #al profe le da 25,a mi 7

#Model for optimal k value
knn.best <-  knn(train=train.gc, test=test.gc, cl=train.gc_labels, k=k.max)

# Checking prediction against actual value in tabular form
table(knn.best ,test.gc_labels)

#Croos-validation with training data set for k=25
knn.cross<-knn.cv(train=train.gc, cl=train.gc_labels, k=k.max,prob=T)
table(knn.cross,train.gc_labels)
confusionMatrix(knn.best ,as.factor(test.gc_labels))

#Alternative calculation of accuracy
sum(knn.best == test.gc_labels)/length(test.gc_labels)*100
