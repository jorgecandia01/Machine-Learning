...
pcontrol <- trainControl(method = "cv", number = 3, 
                       returnResamp = "all", 
...
set.seed(244)
mlp = caret::train(medv ~ .,data = train_,method = "mlpML",
      trControl = pcontrol,
...
pr.nn_tr <- caret::predict.train(mlp, train_[1:13])

# Results from NN are normalized (scaled)
# Descaling for comparison
ds_pr.nn_tr <- pr.nn_tr*(maxs[14]-mins[14])+mins[14]
